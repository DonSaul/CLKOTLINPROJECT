...
export const VacancyFilter = ({
  onFilterChange,
  setData,
  setIsLoadingVacancies,
}) => {
  const [yearsOfExperience, setYearsOfExperience] = useState("");
  const [jobFamily, setJobFamily] = useState("");
  const [salary, setSalary] = useState("");
  const { dataVacancies, refetch, isLoading } = useGetVacancies(
    salary,
    jobFamily,
    yearsOfExperience
  );
    ...
    <div>
      <CardContainer>
        <Typography variant="h6" align="center" gutterBottom>
          <WorkIcon fontSize="large" /> Search Vacancies{" "}
        ...
        <Grid container spacing={2} alignItems="center">
          <Grid item xs={3}>
            <TextField
              label={
                <>
                  <EmojiEventsTwoToneIcon /> Years of Experience
                </>
              }
              type="number"
