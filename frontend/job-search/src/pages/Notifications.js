...
const Notifications = () => {
  ...
  const handleCheckboxNotificationType = useNotificationTypeUpdater(
    getUserEmail()
  );
  ...
  useEffect(() => {
    if (notificationActivated !== null) {
      ...
      const vacancyType = notificationTypes.find(
        (type) => type.id === NOTIFICATION_TYPES.VACANCIES
      );
      const invitationType = notificationTypes.find(
        (type) => type.id === NOTIFICATION_TYPES.INVITATIONS
      );
      const messageType = notificationTypes.find(
        (type) => type.id === NOTIFICATION_TYPES.MESSAGES
      );
        ...
        <CardContainer width="xs">
          <FormControlLabel
            control={
              <Checkbox
                color="primary"
                checked={vacancyChecked}
                onChange={handleVacancyCheckboxChange}
              />
            }
            label="Vacancies"
          />
          <FormControlLabel
            control={
              <Checkbox
                ...
                onChange={handleInvitationCheckboxChange}
              />
            }
            label="Invitations"
          />
          <FormControlLabel
            control={
              <Checkbox
                ...
                onChange={handleMessageCheckboxChange}
              />
            }
            label="Messages"
          />
            ...
            <>
              ...
              <ButtonGroup>
                ...
                <Button style={{ color: "black" }}>
                  {Math.ceil(
                    (notifications ? notifications.length : 0) / itemsPerPage
                  )}
                </Button>
                <Button
                  variant="contained"
